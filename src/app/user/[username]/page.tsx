...
export default function UserTimelinePage() {
  ...
  const [backgroundFetchError, setBackgroundFetchError] = useState(false);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-6 md:mb-8">
          <div className="flex items-center justify-between mb-4">
            <button
              onClick={handleBack}
              className="inline-flex items-center text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 focus:outline-none focus:underline"
            >
              <svg
          ...
          <div className="mb-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-300 dark:border-yellow-700 rounded-lg flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-yellow-800 dark:text-yellow-200">
                Some concerts failed to load in the background
              </p>
              <p className="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
                Loaded concerts are displayed below. Click retry to fetch the remaining concerts.
              </p>
            </div>
            <button
              onClick={retryBackgroundFetch}
              disabled={isLoadingBackground}
              className="flex-shrink-0 px-3 py-1 text-sm font-medium text-yellow-700 dark:text-yellow-300 bg-yellow-100 dark:bg-yellow-800 hover:bg-yellow-200 dark:hover:bg-yellow-700 disabled:opacity-50 disabled:cursor-not-allowed rounded transition-colors"
            >
              {isLoadingBackground ? 'Retrying...' : 'Retry'}
            </button>
          </div>
        )}
